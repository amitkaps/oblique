...
| Bare `italic`, bare `oblique` or `<em>` against a face whose explicit range is `oblique -11deg 11deg` (default synthesis): the axis clamps to -11, but Chrome stacks a synthetic skew (36px lean vs the expected 16px) and Safari drops the axis for italic and `<em>` (pure synthetic skew) or stacks for bare `oblique`. Decided by the reference algorithm | ❌ FAIL | ✅ PASS | ❌ FAIL | [`results/browser-matrix.md`](../results/browser-matrix.md): cells E2, E3, E5. Reproduces a production bug measured independently in a sibling project. Safari results are from a direct `safaridriver` replay, not `wpt run` (`docs/investigation-log.md` section 11) |

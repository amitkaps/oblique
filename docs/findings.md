...
(macOS 15.8). The grid is 6 `@font-face` descriptors (A to F) by 10 use-site requests (1 to 10) on the
...
4 `oblique 11deg`, 5 `font-variation-settings: 'slnt' -11`, 6 `oblique 5deg`, 7 `oblique 45deg`, 8 `oblique -5deg`,
9 `italic` with `font-synthesis-style: none`, 10 `oblique` with `font-synthesis: none`.
...
### Ranges: A2, A3, A7, C2, C3, C7... synthesis stacked on the axis
...
|---|---|---|
| A: the font's own range, `-11deg 11deg` | rows 2, 3, 7 | `normal`, `oblique 11deg`, `5deg`, `-5deg`, `font-variation-settings`, synthesis off |
| C: narrow and one-sided, `0deg 10deg` | the same rows: 2, 3, 7 | the same, including `oblique -5deg`, which clamps to upright |
| B: wider than the font, `-20deg 20deg` | none | all 10 rows, in all three engines |
...
- **It is not the descriptor clamp.** In column C `oblique 11deg` is outside the range yet passes, while `italic` (which
  ...
  numbers (`slnt -10`), and its one new probe, a negative request clamped to the range's edge (C8), passes everywhere.
- **Declaring a range wider than the font's own avoids the stacked skew in all three engines.** This is the practical
  workaround. It rests on the 60 measured cells per engine, not on reading Chrome's or WebKit's source.
- Rows 9 and 10 repeat rows 2 and 3 with `font-synthesis-style: none` / `font-synthesis: none` and pass everywhere, so
...
|---|---|---|
| specified | one testable outcome; a reftest with one `match` reference | 52 |
...
`mise run compare`: 54 of 60 cells agree, 6 diverge (the six failing cells above), none is `reference-suspect` (a cell every
...
- `auto` (descriptor omitted): §4.4 scopes its two clauses separately. For *selection* the face is treated as
  ...
  `auto` face stays open (§5.2's italic steps never set `slnt`), so F2 and F9 are only observed: Chrome and
...
- A variable font's axis is not synthesis: §2.8 says such fonts "do not count as font synthesis and their use is
  ...
  never disables the axis (A9, A10 and F10 stay on it in all three engines), and an out-of-range angle on an `auto`
...
- Rows 1 to 8 never set `font-synthesis`: the spec synthesizes only for a missing face, so forcing it off
  would hide an engine that synthesizes on top of a matched face. Rows 9 and 10 exist to set it.
...
Cells E3, E4, E6, E7 and E8 allow upright or a synthesized skew and forbid the real axis. The reading that gives
...
- **Literal §5.2:** the slnt step has no exception for a range restricted to 0, so the axis match is made, clamped
  to 0, and shearing is never reached. That makes E3 to E8 upright only, and all three engines fail them.
...
1. **Grow the grid, sparingly.** Each new column must exercise a behaviour no column has yet. Not adding: a
   ...
   and `-11deg` (same outcome as rows 6 and 8 on one face) and a `font-variation-settings` row with a non-normal request
   (it always wins by the cascade, and row 5 covers it). Addresses were renumbered on 2026-09-20 when italic fonts
...
5. **File the spec questions** with CSSWG, most valuable first:
   - Synthesis "will be generated" (2.3) versus a UA "may create" it (5.2). This is the largest pool of untested
     latitude: E3, E4, E6, E7 and E8, where synthesis is permitted but not required. Making it mandatory would add
     "must not be upright" to those five and newly fail Chrome and Safari on E4, E6 and E8, which render the requested

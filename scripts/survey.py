...
capital I: how far the top of the stem is displaced from its bottom, in CSS pixels.

Screenshots come back in DEVICE pixels, so on a HiDPI display every lean would be
scaled (safaridriver on a 2x screen doubles them; chromedriver and geckodriver
headless do not). Each screenshot is therefore divided by the scale measured from
that screenshot itself - its width over window.innerWidth - so a run is comparable
across machines and monitors. The scale is recorded per engine.
...
Usage (from repo root):
  ...
  uv run scripts/survey.py --engines safari         # merged into the engines already recorded
...
<p class="test"{style}>{font['glyph']}</p>"""


def lean_of(png):
    """(lean in screenshot px, screenshot width) for one probe, or (None, width) if no glyph."""
    ...
    return left(top) - left(bottom), shot_w
            ...
            for _ in range(reps):
                ...
                while time.time() < deadline:
                    ...
                    if not waiting:
                        break
                    time.sleep(0.05)
                lean, shot_w = lean_of(base64.b64decode(wd(port, "GET", f"/session/{sid}/screenshot")))
                if scale is None:
                    inner = wd(port, "POST", f"/session/{sid}/execute/sync",
                               {"script": "return window.innerWidth", "args": []})
                    raw = shot_w / inner
                    scale = round(raw * 2) / 2  # real displays are 1x, 1.5x, 2x, ...
                    if abs(raw - scale) > 0.05 * scale:
                        print(f"  {engine}: screenshot {shot_w}px / innerWidth {inner}px = {raw:.3f}, "
                              f"not a display scale; using {scale}", file=sys.stderr)
                    if scale != 1:
                        print(f"  {engine}: screenshots are {scale}x device pixels, dividing", file=sys.stderr)
                leans.append(None if lean is None else round(lean / scale))
            if len(set(leans)) != 1:
                print(f"  {engine} {cell['address']}: unstable {leans}, not recorded", file=sys.stderr)
                continue
            cells[cell["id"]] = leans[0]
        wd(port, "DELETE", f"/session/{sid}")
        return {"version": version, "scale": scale, "cells": cells}
    finally:
        driver.terminate()
...
def main():
    ...
    time.sleep(1)
    # engines not surveyed in this run keep the rows they already had
    previous = json.loads(OUT.read_text())["engines"] if OUT.exists() else {}
    result = {"_note": "Generated by scripts/survey.py: measured lean (CSS px) per generated cell; "
                       "`scale` is the device-pixel scale the screenshots were divided by.",
              "engines": previous}
    try:

...
```
matrix-*.html           58 generated reftests (and their -ref / -notref files), never edited by hand
...
5 `font-variation-settings: 'slnt' -11`, 6 `oblique 5deg`, 7 `oblique 45deg`, 8 `oblique -5deg`, 9 `italic` with
`font-synthesis-style: none`, 10 `oblique` with `font-synthesis: none`. A cell is `A2`; its file is
...
descriptor under test. Only rows 9 and 10 set `font-synthesis`; that is their point. Results and what they mean:

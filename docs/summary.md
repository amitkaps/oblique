...
| Bare `italic`, bare `oblique` or `<em>` against a face whose explicit range is `oblique -11deg 11deg` (default synthesis): axis clamps, but Chrome *also* stacks a synthetic skew (3 grid cells, plus the older hand-written stacking test) | ❌ FAIL | ✅ PASS | not run | [`results/browser-matrix.md`](../results/browser-matrix.md): 36px lean measured vs the expected 16px; reproduces a real production bug measured independently in a sibling project |
| `italic` or `<em>` against a face declared `font-style: italic` on a `slnt`-only font (no `ital` axis): Chrome drives the real axis anyway | ❌ FAIL | ✅ PASS | not run | [`results/browser-matrix.md`](../results/browser-matrix.md): 2 grid cells; same polarity as `italic-no-extra-synthesis.html` on a different font shape |
| `oblique 11deg` against a face declared `font-style: normal`: Chrome renders upright, Firefox synthesizes | ❌ FAIL | ✅ PASS | not run | [`results/browser-matrix.md`](../results/browser-matrix.md): bare keywords synthesize on both engines, the explicit angle does not on Chrome |

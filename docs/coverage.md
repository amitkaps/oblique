...
**Candidate test:** Done — see tests/oblique-style-matching/auto-derived-range-clamp.html. If Cairo's own bug is worth re-verifying independently, the next step would be a variant using a font with Cairo's exact symmetric -11..11 range (resources/oblique-symmetric.ttf, built 2026-09-18 for this same folder's boundary tests, already has this exact shape) rather than Inter's one-sided range, to rule out that specific difference.

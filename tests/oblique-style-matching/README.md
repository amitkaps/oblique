...
## The `matrix-face-*-use-*.html` grid (2026-09-19)

30 tests, one per cell of a 5 x 6 grid, generated by `resources/gen-matrix.py`
(its docstring and per-cell notes are the record of how each expected value
was derived). Columns are what `@font-face` declares: `auto` (no descriptor),
`normal`, `italic`, bare `oblique`, `oblique -11deg 11deg`. Rows are the use-site
CSS: `font-style: normal`, `italic`, `oblique`, `oblique 11deg`, `<em>`, and
`font-variation-settings: 'slnt' -11`. All use `resources/Cairo.var.subset.ttf`
and the capital `I`. The site's coverage matrix renders exactly this grid.

**No test sets `font-synthesis`.** The spec synthesizes only when a family
lacks a suitable face, so a request that a declared face satisfies must never
synthesize; leaving it at the default is the stricter, more realistic test and
the only one that can see a browser synthesizing anyway. (The older
hand-written tests, `boundary-11deg-*`, `auto-derived-range-clamp*` and
`style-plus-explicit-variation-settings`, still force `font-synthesis: none` to
isolate the clamp or precedence question they were written for; they sit
outside the grid.)

A face declared `normal` counts as having no oblique face however the font's
own `fvar` looks: the descriptor replaces the style implied by the font data
(CSS Fonts 4 section 4.4). Its italic/oblique/`<em>` cells therefore expect a
synthesized skew, asserted with two `rel="mismatch"` references (not upright,
not the real axis).

Result: 24 of 30 pass on both engines. Six fail on Chrome only, and Firefox
passes all 30: the explicit-range face with bare italic, bare oblique and
`<em>` (the synthesis-stacking bug), the italic-declared face with italic and

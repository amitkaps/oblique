...
## The `matrix-*.html` batch (2026-09-19)

25 test/ref pairs systematically enumerating every `@font-face font-style`
descriptor value (`auto`, `normal`, `italic`, bare `oblique`,
`oblique <range>`) crossed with every use-site CSS pattern this repo's
tests exercise (bare `italic`, bare `oblique` at default/suppressed
synthesis, explicit `oblique <angle>`, `<em>`, direct
`font-variation-settings: 'slnt'`, and `normal`) — all against
`resources/Cairo.var.subset.ttf` (the capital `I`), generated by `resources/gen25.py` (kept
as the record of how each reference was derived; see its own docstring).
Every cell's expected value was measured first (pixel shear comparison,
Chrome + Firefox), not assumed. 24 of 25 are consistent, confirmed
coverage; one — `matrix-italic-r5-italic.html` — is a genuine, isolated
cross-engine divergence, the same Chrome-over-applies/Firefox-correct
polarity `italic-no-extra-synthesis.html` already documents, reproduced
independently on a different font shape. Full writeup:
`docs/investigation-log.md`'s section 8.
